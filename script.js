...
map.getPane("playerPane").style.zIndex = 650;
	...
	// 캔버스를 지도의 부모 요소로 이동시켜 지도 위에 덮이게 함
...
function renderFog() {
    ...
    // 1. 전체를 안개로 가득 채움 (가장 어두운 상태)
    fogCtx.fillStyle = "rgba(8, 10, 18, " + getFogAlpha() + ")";
    fogCtx.fillRect(0, 0, w, h);
    
    // 2. 현재 위치 주변은 항상 조금 보임 (플레이어 주변 시야)
    if (currentPos) {
        var mpp = calcMpp();
    ...
    var offscreen = document.createElement("canvas"); offscreen.width = w; offscreen.height = h; var offCtx = offscreen.getContext("2d");
    buckets.forEach(function(drawFns, alpha) {
        offCtx.clearRect(0, 0, w, h); 
        offCtx.fillStyle = "rgba(0,0,0," + alpha + ")"; 
        offCtx.strokeStyle = "rgba(0,0,0," + alpha + ")";
        offCtx.lineWidth = radius * 2.2; // 선 두께를 약간 더 두껍게
...
function renderAgeTint() {
    ...
    buckets.forEach(function(draws, color) {
        ...
        draws.forEach(function(d) { offCtx.moveTo(d.x1, d.y1); offCtx.lineTo(d.x2, d.y2); }); offCtx.stroke(); ageCtx.drawImage(offscreen, 0, 0);
    });
}
...
function updatePhotoMarkerSizes() { var size = getPhotoMarkerSize(); photoClusterGroup.eachLayer(function(marker) { if (marker._photoData) { marker.setIcon(L.divIcon({ className: "photo-marker", html: '<img src="' + marker._photoData.thumb + '" style="width:' + size + 'px;height:' + size + 'px;object-fit:cover;border-radius:6px;border:2px solid #fff;" />', iconSize: [size, size], iconAnchor: [size / 2, size] })); } }); }
...
async function triggerCamera() { closePhotoMenu(); if (window.Capacitor && window.Capacitor.isNativePlatform()) { try { var Camera = window.Capacitor.Plugins.Camera; var photo = await Camera.getPhoto({ quality: 80, resultType: "uri", source: "CAMERA" }); var now = new Date(); var img = new Image(); img.onload = function() { var lat = currentPos ? currentPos.lat : map.getCenter().lat; var lng = currentPos ? currentPos.lng : map.getCenter().lng; processPhoto(img, now, lat, lng); }; img.src = photo.webPath; } catch (e) { console.warn("카메라 실패", e); } } else { document.getElementById("camera-input").click(); } }
...
function switchTab(tab) { ["memory", "photo", "gpx"].forEach(function(t) { document.getElementById("tab-" + t).classList.toggle("active", t === tab); document.getElementById("panel-" + t).style.display = t === tab ? "" : "none"; }); if (tab === "photo") updatePhotoList(); if (tab === "gpx") updateGpxSavedList(); }
...
function persistState() { try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ pathCoordinates: pathCoordinates.map(function(p) { return { lat: p.lat, lng: p.lng, startTime: p.startTime, endTime: p.endTime, visits: p.visits || 1 }; }), memories: memories.map(function(m) { return { id: m.id, lat: m.lat, lng: m.lng, name: m.name, time: m.time, dateString: m.dateString, timeString: m.timeString }; }), photos: photos.map(function(p) { return { id: p.id, lat: p.lat, lng: p.lng, time: p.time, dateString: p.dateString, timeString: p.timeString }; }), totalDistance: totalDistance })); } catch (e) { console.error("저장 실패", e); if (e && e.name === "QuotaExceededError") { alert("저장 공간이 부족합니다."); } } }

function loadState() { try { var raw = localStorage.getItem(STORAGE_KEY); if (!raw) return; var saved = JSON.parse(raw); if (Array.isArray(saved.pathCoordinates)) { pathCoordinates = saved.pathCoordinates.filter(function(p) { return isFinite(p.lat) && isFinite(p.lng) && isFinite(p.startTime) && isFinite(p.endTime); }).map(function(p) { return { lat: p.lat, lng: p.lng, startTime: p.startTime, endTime: p.endTime, visits: isFinite(p.visits) ? p.visits : 1 }; }); } if (Array.isArray(saved.memories)) { memories = saved.memories.filter(function(m) { return isFinite(m.lat) && isFinite(m.lng) && typeof m.name === "string"; }).map(function(m) { return { id: typeof m.id === "string" ? m.id : String(m.time), lat: m.lat, lng: m.lng, name: m.name, time: m.time, dateString: m.dateString, timeString: typeof m.timeString === "string" ? m.timeString : new Date(m.time).toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" }) }; }); } if (isFinite(saved.totalDistance)) totalDistance = saved.totalDistance; if (Array.isArray(saved.photos)) { photos = saved.photos.filter(function(p) { return isFinite(p.lat) && isFinite(p.lng) && p.id; }); } var savedFog = localStorage.getItem(FOG_ENABLED_KEY); if (savedFog !== null) isFogEnabled = savedFog === "true"; compactPathData(); } catch (e) { console.error("복원 실패", e); } }

// ── 사진 처리 ──
function processPhoto(img, now, lat, lng) { var popup = resizeImage(img, 200); var thumb = resizeImage(img, 40); var id = String(now.getTime()) + Math.random().toString(36).slice(2); var data = { id: id, lat: lat, lng: lng, photo: popup, thumb: thumb, time: now.getTime(), dateString: now.toLocaleDateString("ko-KR", { year:"numeric", month:"long", day:"numeric" }), timeString: now.toLocaleTimeString("ko-KR", { hour:"2-digit", minute:"2-digit" }) }; photos.push(data); idbSavePhoto(id, popup, thumb).catch(function(e) { console.warn("IDB 저장 실패", e); }); createPhotoMarker(data, true); updateStats(); scheduleSave(); updatePhotoList(); }
function handlePhotos(event) { var files = Array.from(event.target.files); if (!files.length) return; var processed = 0; var finishOne = function() { processed++; if (processed === files.length) { updateStats(); scheduleSave(); updatePhotoList(); event.target.value = ""; } }; files.forEach(function(file) { EXIF.getData(file, function() { var lat = null, lng = null; var latVal = EXIF.getTag(this, "GPSLatitude"); var latRef = EXIF.getTag(this, "GPSLatitudeRef"); var lngVal = EXIF.getTag(this, "GPSLongitude"); var lngRef = EXIF.getTag(this, "GPSLongitudeRef"); if (latVal && lngVal) { lat = latVal[0] + latVal[1]/60 + latVal[2]/3600; lng = lngVal[0] + lngVal[1]/60 + lngVal[2]/3600; if (latRef === "S") lat = -lat; if (lngRef === "W") lng = -lng; } if (!lat || !lng) { if (currentPos) { lat = currentPos.lat; lng = currentPos.lng; } else { var center = map.getCenter(); lat = center.lat; lng = center.lng; } } var reader = new FileReader(); reader.onload = function(e) { var now = new Date(); var img = new Image(); img.onerror = function() { fetch(e.target.result).then(function(r) { return r.blob(); }).then(function(blob) { if (typeof heic2any !== "undefined") { return heic2any({ blob: blob, toType: "image/jpeg", quality: 0.85 }); } throw new Error("heic2any 없음"); }).then(function(jpegBlob) { var url = URL.createObjectURL(jpegBlob); var img2 = new Image(); img2.onload = function() { processPhoto(img2, now, lat, lng); finishOne(); URL.revokeObjectURL(url); }; img2.src = url; }).catch(function() { finishOne(); }); }; img.onload = function() { processPhoto(img, now, lat, lng); finishOne(); }; img.src = e.target.result; }; reader.readAsDataURL(file); }); }); }
function resizeImage(img, maxSize) { var canvas = document.createElement("canvas"); var w = img.width, h = img.height; if (w > h && w > maxSize) { h = Math.round(h * maxSize / w); w = maxSize; } else if (h > maxSize) { w = Math.round(w * maxSize / h); h = maxSize; } canvas.width = w; canvas.height = h; canvas.getContext("2d").drawImage(img, 0, 0, w, h); return canvas.toDataURL("image/jpeg", 0.7); }
function createPhotoMarker(data, openPopup) { var size = getPhotoMarkerSize(); var marker = L.marker([data.lat, data.lng], { pane: "photoPane", icon: L.divIcon({ className: "photo-marker", html: '<img src="' + data.thumb + '" style="width:' + size + 'px;height:' + size + 'px;object-fit:cover;border-radius:6px;border:2px solid #fff;" />', iconSize: [size, size], iconAnchor: [size / 2, size] }) }); marker._photoData = data; var popupEl = document.createElement("div"); popupEl.className = "photo-popup"; var img = document.createElement("img"); img.src = data.photo; img.style.cssText = "width:200px;border-radius:8px;margin-bottom:8px;display:block;"; var info = document.createElement("div"); info.style.cssText = "font-size:12px;color:rgba(255,255,255,0.6);text-align:center;margin:6px 0 8px;"; info.textContent = data.dateString + " " + data.timeString; var delBtn = document.createElement("button"); delBtn.className = "popup-delete-btn"; delBtn.textContent = "사진 삭제"; delBtn.addEventListener("click", function() { deletePhoto(data.id); marker.closePopup(); }); popupEl.appendChild(img); popupEl.appendChild(info); popupEl.appendChild(delBtn); marker.bindPopup(popupEl); photoClusterGroup.addLayer(marker); if (openPopup) marker.openPopup(); }
...
function renderStoredPhotoMarkers() { if (photos.length === 0) return; idbGetAllPhotos().then(function(idbList) { var idbMap = new Map(idbList.map(function(r) { return [r.id, r]; })); photos.forEach(function(p) { var img = idbMap.get(p.id); if (img) { p.photo = img.photo; p.thumb = img.thumb; createPhotoMarker(p, false); } }); }).catch(function(e) { console.warn("IDB 불러오기 실패", e); }); }
...
function fetchFestivals() {
    ...
    var url = FESTIVAL_ENDPOINT + "?serviceKey=" + TOUR_API_KEY
        + "&eventStartDate=" + today
        + "&mapX=" + center.lng.toFixed(6)
        + "&mapY=" + center.lat.toFixed(6)
        + "&radius=50000"
        + "&numOfRows=20&pageNo=1&MobileOS=ETC&MobileApp=Giloa&_type=json&arrange=E";
...
function updateFestivalBadge() {
    ...
    if (festivalItems.length > 0) {
        badge.textContent = "🎪 축제 " + festivalItems.length;
...
function renderFestivalStrip() {
    ...
    festivalItems.forEach(function(item) {
        ...
        nameEl.className = "festival-card-name";
        nameEl.textContent = item.title || "축제";
        var dateEl = document.createElement("div");
        dateEl.className = "festival-card-date";
        var start = item.eventstartdate || "";
        var end = item.eventenddate || "";
        ...
        distEl.className = "festival-card-dist";
        ...
        card.addEventListener("click", function() {
            ...
            L.popup({ className: "tour-popup" })
                ...
                .setContent("<b>" + escapeHtml(item.title || "") + "</b><br><small style='color:#4db8ff;'>🎪 축제</small><br><small>" + escapeHtml(addr) + "</small>" + tel)
                .openOn(map);
        });
...
function fetchTourSpots() {
    ...
    loadingEl.style.display = ""; emptyEl.style.display = "none"; listEl.innerHTML = ""; expandBtn.style.display = "none";
    ...
    fetch(url).then(function(res) { return res.json(); }).then(function(data) {
        ...
        if (items.length === 0) { emptyEl.style.display = ""; countEl.textContent = ""; return; }
        countEl.textContent = items.length + "곳"; renderTourCards();
    }).catch(function(err) { loadingEl.style.display = "none"; emptyEl.style.display = ""; emptyEl.textContent = "불러오기 실패"; countEl.textContent = ""; console.warn("TourAPI 에러", err); });
}
...
function renderTourCards() {
    ...
    if (tourItems.length > 0 && tourExpanded) { expandBtn.style.display = ""; document.getElementById("tour-expand-icon").textContent = "▲"; document.getElementById("tour-expand-text").textContent = "접기"; listEl.classList.add("expanded"); }
    else { expandBtn.style.display = "none"; listEl.classList.remove("expanded"); }
    addTourMarkers();
    if (tourExpanded) renderFestivalStrip();
}
...
function showTourPopup(item) {
    ...
    var typeName = TOUR_TYPE_NAMES[item.contenttypeid] || "관광"; var addr = item.addr1 || "";
    var tel = item.tel ? "<br><a href='tel:" + item.tel + "' style='color:#4db8ff;font-size:12px;'>📞 " + escapeHtml(item.tel) + "</a>" : "";
    L.popup({ className: "tour-popup" }).setLatLng([lat, lng]).setContent("<b>" + escapeHtml(item.title || "") + "</b><br><small style='color:rgba(120,220,140,0.9);'>" + typeName + "</small><br><small>" + escapeHtml(addr) + "</small>" + tel).openOn(map);
}
...
map.on("click", function() { if (tourExpanded) { tourExpanded = false; renderTourCards(); } });
scheduleTourFetch();
// ── 시야각 부채꼴 ──
var visionCone = null;
